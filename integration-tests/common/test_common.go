...
func (testState *Test) DeployCluster() {
	...
	if !testState.Common.Testnet { // devnet!
		// plugin starknet client needs the RPC API url which is at /rpc on devnet
		baseURL += "/rpc"
	}
...
func (testState *Test) GetPluginNodes() []*client.PluginClient {
	// retrieve client from K8s client
	pluginNodes := []*client.PluginClient{}
	for i := range testState.Cc.PluginNodes {
		pluginNodes = append(pluginNodes, testState.Cc.PluginNodes[i].PluginClient)
	}
	return pluginNodes
}
