...
func (m *OCRv2TestState) DeployCluster() {
	...
	if *m.Common.TestConfig.Common.InsideK8s {
		...
		m.Clients.PluginClient.PluginNodes = m.Clients.DockerEnv.ClCluster.NodeAPIs()
		m.Clients.PluginClient.NKeys, m.TestConfig.err = m.Common.CreateNodeKeysBundle(m.Clients.DockerEnv.ClCluster.NodeAPIs())
		require.NoError(m.TestConfig.T, m.TestConfig.err)
	}
	lggr := logger.Nop()
