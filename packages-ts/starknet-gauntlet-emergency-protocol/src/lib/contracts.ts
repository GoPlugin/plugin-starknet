...
export const uptimeFeedContractLoader = (): CompiledContract => {
  return json.parse(
    fs
      .readFileSync(
        `${__dirname}/../../../../contracts/starknet-artifacts/src/plugin/cairo/emergency/SequencerUptimeFeed/sequencer_uptime_feed.cairo/sequencer_uptime_feed.json`,
      )
      .toString('ascii'),
  )
}
...
export const starknetValidatorContractLoader = (): ContractFactory => {
  const abi = JSON.parse(
    fs
      .readFileSync(
        `${__dirname}/../../../../contracts/artifacts/src/plugin/solidity/emergency/StarknetValidator.sol/StarknetValidator.json`,
      )
      .toString('ascii'),
  )
