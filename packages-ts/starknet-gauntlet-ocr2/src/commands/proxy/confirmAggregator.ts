...
const beforeExecute: BeforeExecute<UserInput, ContractInput> = (
  ...
  deps.logger.info(
    `About to confirm aggregator ${input.user.address} for proxy at ${input.contract.join(', ')}`,
  )
}
...
const commandConfig: ExecuteCommandConfig<UserInput, ContractInput> = {
  ...
  action: 'confirm_aggregator',
  ux: {
    description: 'Confirm new aggregator for OCR2 proxy',
    examples: [
      `${CATEGORIES.PROXY}:confirm_aggregator --network=<NETWORK> --address=<AGGREGATOR> <PROXY>`,
    ],
  },
  makeUserInput,
  makeContractInput,
