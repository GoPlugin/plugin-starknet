...
export const loadContract_Ocr2 = (name: CONTRACT_LIST): CompiledContract => {
  return json.parse(
    fs
      .readFileSync(
        `${__dirname}/../../../../contracts/starknet-artifacts/src/plugin/cairo/ocr2/${name}.cairo/${name}.json`,
      )
      .toString('ascii'),
  )
}
...
export const loadContract_Example = (name: CONTRACT_LIST): CompiledContract => {
  return json.parse(
    fs
      .readFileSync(
        `${__dirname}/../../../../contracts/starknet-artifacts/src/plugin/cairo/example/${name}.cairo/${name}.json`,
      )
      .toString('ascii'),
  )
}
...
export const loadContract_AccessController = (name: CONTRACT_LIST): CompiledContract => {
  return json.parse(
    fs
      .readFileSync(
        `${__dirname}/../../../../contracts/starknet-artifacts/src/plugin/cairo/access/SimpleWriteAccessController/${name}.cairo/${name}.json`,
      )
      .toString('ascii'),
  )
}
