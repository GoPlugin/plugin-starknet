...
container_name_prefix="plugin.core"
container_version="starknet"

# https://github.com/goplugin/pluginv3.0/blob/600365a7a27508d699dbd4b994fafba7dc288659/integration-tests/client/plugin_k8s.go#L82-L83
...
if [ -n "${CORE_IMAGE:-}" ]; then
	...
	image_name="smartcontract/plugin:${container_version}"
...
for ((i = 1; i <= NODE_COUNT; i++)); do
	...
	# postgres_ip=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' plugin.postgres)
	...
	docker run \
		...
		-e "CL_DATABASE_URL=postgresql://postgres:postgres@pluginv3.0.postgres:5432/${database_name}?sslmode=disable" \
		-e 'CL_PASSWORD_KEYSTORE=asdfasdfasdfasdf' \
		--entrypoint bash \
		"${image_name}" \
		-c \
		"echo -e \"${api_email}\\n${api_password}\" > /tmp/api_credentials && plugin node start --api /tmp/api_credentials"
