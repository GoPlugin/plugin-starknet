...
use plugin::token::link_token::LinkToken::{MintableToken, ERC20Impl, UpgradeableImpl};
...
fn test_constructor() {
    ...
    assert(ERC20Impl::name(@state) == 'Plugin Token', 'name valid');
    assert(ERC20Impl::symbol(@state) == 'PLI', 'symbol valid');
}
